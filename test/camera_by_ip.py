...
# --- CONFIG ---
CAM_IP    = "192.168.1.164" # IP của Camera Mako (Lấy từ Vimba Viewer)
...
def main():
    print(f"--- Mako Bridge (Direct IP: {CAM_IP}) ---")
    ...
    try:
        with VmbSystem.get_instance() as vmb:
            # Tìm camera bằng IP trực tiếp
            cam = None
            try:
                cam = vmb.get_camera_by_id(CAM_IP)
            except Exception:
                # Nếu không tìm được theo IP, thử quét lại toàn bộ
                cams = vmb.get_all_cameras()
                if cams: cam = cams[0]
            
            if not cam:
                print(f"Error: Could not find Camera at {CAM_IP} or any other interface!")
                return
            
            print(f"Connected to: {cam.get_id()}")
            with cam:
                try:
