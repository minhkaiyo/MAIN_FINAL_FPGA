...
wire gvcp_done;
wire tx_start_mux  = tx_start_arp ? tx_start_arp : tx_start_gvcp;
wire [10:0] tx_len_mux = tx_start_arp ? tx_len_arp : tx_len_gvcp;
wire [7:0]  buf_data_mux = tx_start_arp ? buf_data_arp : buf_data_gvcp;

eth_tx_mii tx_eng (
    .clk_25(ENET_TX_CLK), .rst_n(phy_rst_n),
...
arp_responder arp_mod (
    .clk_25(ENET_TX_CLK), .rst_n(phy_rst_n), .link_up(link_up),
...
gvsp_rx gvsp_mod (
    ...
    .pixel_addr(pixel_addr_gvsp), .pkt_count(LEDG[7:0])
);
...
wire [7:0] rd_idx = h_cnt[9:2]; // 2x horizontal scale
...
// ==========================================================================
// 9. SDRAM FSM (giong v2, giu nguyen)
// ==========================================================================
...
always @(posedge clk or negedge pll_locked) begin
    if (!pll_locked) begin
        ...
        dq_oe<=0; buf_wr_en<=0; refresh_cnt<=0; fifo_rd<=0;
    end else begin
...
// 10. DEBUG
// ==========================================================================
assign LEDR[0]=fifo_full; assign LEDR[1]=gvcp_done;
assign LEDR[2]=link_up;   assign LEDR[17:3]=0;
...
assign HEX3=seg7(burst_count[6:4]);assign HEX2=seg7(burst_count[3:0]);
assign HEX1=seg7(0); assign HEX0=seg7({3'd0,gvcp_done});
